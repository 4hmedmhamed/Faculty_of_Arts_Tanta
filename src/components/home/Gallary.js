...
function Scene({ children, ...props }) {
  ...
  return (
    <group ref={ref} {...props}>
      <Cards category="الرفع" from={0} len={Math.PI / 4} onPointerOver={hover} onPointerOut={hover} />
      <Cards category="التقرير" from={Math.PI / 4} len={Math.PI / 2} position={[0, 0.4,0]} onPointerOver={hover} onPointerOut={hover} />
      <Cards category="التصحيح" from={Math.PI / 4 + Math.PI / 2} len={Math.PI / 2} onPointerOver={hover} onPointerOut={hover} />
      <Cards category="التحقق" from={Math.PI * 1.25} len={Math.PI * 2 - Math.PI * 1.25} position={[0, -0.4, 0]} onPointerOver={hover} onPointerOut={hover} />
...
function ActiveCard({ hovered, ...props }) {
  ...
  return (
    <Billboard {...props}>
      ...
      <Image  className="w-100"  ref={ref} transparent radius={0.9} position={[0, 1.5, 0]} scale={[5.5, 1.618 * 3.5, 1.2, 1]} url={`/${Math.floor(hovered % 10) + 1}.jpg`} />
    </Billboard>
  )
}
